...
| Emperor | 4 | **Enforced** | **Enforced** | `AdeptRules.TryProtectSpreadCards` (Spring activation; hand+spread when attuned) + `CombatRules` / `AdeptRules.TryDevilSteal` | Active Effects badge → Emperor modal | Yes | `RuleServices_Tests.Emperor_*`, `EmperorProtectionBindings_Tests` |
