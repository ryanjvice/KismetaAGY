...
namespace Kismeta.Game.Bootstrap
{
    ...
    public sealed class GameDebugUI : MonoBehaviour
    {
        ...
        private string FormatEvent(IGameEvent evt) => evt switch
        {
            ...
            DuelResolvedEvent e         => $"[Duel] P{e.AttackerId}({e.AttackerRoundWins}) vs P{e.DefenderId}({e.DefenderRoundWins}) → Winner:P{e.WinnerId}",
            GambitResolvedEvent e       => $"[Gambit] P{e.AttackerId}({e.AttackerRoundWins}) vs P{e.DefenderId}({e.DefenderRoundWins}) → Winner:P{e.WinnerId}",
